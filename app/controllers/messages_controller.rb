  ...
  def create
    ...
    if message.save
      respond_to do |format|
        ...
        format.json { render json: message.create_json.merge(image: message.image.url, message_id: message.id) }
      end
    else
...
def get_update_messages
    update_messages = ChatGroup.find(params[:chat_group_id]).messages.where('id = 582 ')
end
...
def get_create_json(get_update_messages, get_update_user_nicknames)
  array = []
  array = array.push(get_update_messages)
  array = array.push(get_update_user_nicknames)
  return array
end
